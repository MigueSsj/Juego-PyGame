...
# ---------------------------------------------------------------------
# Nivel 1 – Parque (Fácil, sin tiempo) — Guardianes del Planeta
# Dinámica:
#   - Solo puedes cargar UNA basura a la vez (E / Enter).
#   - La llevas al BASURERO y la depositas (E / Enter cerca del basurero).
#   - Ganas al depositar todas.
#   - Pausa con ESPACIO: Continuar / Reiniciar / Menú.
# ---------------------------------------------------------------------
...
def load_trash_images(assets_dir: Path) -> list[pygame.Surface]:
    ...
    # Soporta tus nombres: trash_envase, trash_lata, trash_piedra, etc.
...
def load_char_frames(assets_dir: Path, target_h: int) -> dict[str, list[pygame.Surface]]:
    ...
    def scale_list(lst: list[pygame.Surface]) -> list[pygame.Surface]:
        out = []
        for f in lst:
            w = int(f.get_width() * (target_h / f.get_height()))
            out.append(pygame.transform.smoothscale(f, (w, target_h)))
        return out
...
def run(screen: pygame.Surface, assets_dir: Path, personaje: str = "EcoGuardian", dificultad: str = "Fácil"):
    ...
    # Basuras (usa tus trash_* si existen)
    ...
    while True:
        ...
        for e in pygame.event.get():
            ...
            if e.type == pygame.KEYDOWN:
                ...
                if e.key in PICK_KEYS:       interact = True
        ...
        if not paused:
            ...
            # Mantener basura cargada pegada al jugador
            if carrying:
            ...
            if interact:
                if not carrying:
                    ...
                    # DEPOSITAR: si está cerca del basurero
        ...
        if back_img:
            ...
            screen.blit(font.render("Menú", True, (10, 10, 10)),
                        font.render("Menú", True, (10, 10, 10)).get_rect(center=back_rect.center))
