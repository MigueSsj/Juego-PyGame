...
def run(screen: pygame.Surface, assets_dir: Path, personaje: str = "EcoGuardian", dificultad: str = "Fácil"):
    ...
    while True:
        ...
        if paused:
            ...
            btn_w, btn_h = int(panel_w2 * 0.70), int(panel_h2 * 0.17)
            cx2 = W // 2
            start_y = panel2.top + int(panel_h2 * 0.28)
            gap = int(panel_h2 * 0.06)
            r_cont    = pygame.Rect(0, 0, btn_w, btn_h); r_cont.center    = (cx2, start_y)
            r_restart = pygame.Rect(0, 0, btn_w, btn_h); r_restart.center = (cx2, start_y + btn_h + gap)
            r_menu    = pygame.Rect(0, 0, btn_w, btn_h); r_menu.center    = (cx2, start_y + (btn_h + gap) * 2)
